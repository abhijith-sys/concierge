import { BusinessStatus, Role, HomeTradeEnquiryStatus, type Prisma } from "@prisma/client";
import { ApiError } from "../../shared/errors/index.js";
import { isHomeServiceCategorySlug } from "../../shared/domain/home.js";
...
import type {
  ...
  HomeTradeEnquiryListQuery,
  UpdateHomeTradeEnquiryInput,
...
export const homeTradeEnquiriesService = {
  async create(input: CreateHomeTradeEnquiryInput, user?: AuthUser, ctx?: { ip?: string; requestId?: string }) {
    const business = await homeTradeEnquiriesRepository.findBusinessForEnquiry(input.businessId);
    if (!business || business.status !== BusinessStatus.active || !business.listing) {
      throw new ApiError(404, "BUSINESS_NOT_FOUND", "This home trade provider is not available");
    }
    if (!homeSlugsOf(business).some(isHomeServiceCategorySlug)) {
      throw new ApiError(
        400,
        "NOT_HOME_TRADE",
        "Enquiries of this type are only for home and property trade listings",
      );
    }

    const requestedIds = [...new Set(input.serviceSelections.map((row) => row.serviceId))];
    const services = await homeTradeEnquiriesRepository.findApprovedServices(business.id, requestedIds);
    ...
    const enquiry = await homeTradeEnquiriesRepository.create({
    ...
    await writeAuditLog({
      actorId: user?.id,
      action: "home_trade_enquiry.create",
      entityType: "home_trade_enquiry",
      entityId: enquiry.id,
    ...
    try {
      await EmailService.send({
        to: business.email,
        subject: `New home trade enquiry for ${business.name}`,
        body: [
          `A customer requested a home trade job from ${business.name}.`,
          "",
  ...
  async listForActor(query: HomeTradeEnquiryListQuery, user: AuthUser) {
    const where: Prisma.HomeTradeEnquiryWhereInput = {
    ...
    const [items, total] = await Promise.all([
      homeTradeEnquiriesRepository.list(where, skip, query.pageSize),
      homeTradeEnquiriesRepository.count(where),
    ]);
    return {
      items,
      pagination: {
  ...
  async update(id: string, input: UpdateHomeTradeEnquiryInput, user: AuthUser) {
    const existing = await homeTradeEnquiriesRepository.findById(id);
    if (!existing) throw new ApiError(404, "ENQUIRY_NOT_FOUND", "Home trade enquiry not found");
