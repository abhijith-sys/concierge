...
export function AutomotiveEnquiriesPage() {
  const { user } = useAuth();
  const queryClient = useQueryClient();
  const params = new URLSearchParams({ pageSize: "50" });
  const list = useQuery({
    queryKey: ["admin", "automotive-enquiries", params.toString()],
    queryFn: () => api.automotiveEnquiries(params),
  ...
  const update = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["admin", "automotive-enquiries"] });
    },
  });
  ...
  return (
    <div className="stack">
      <div>
        <h2 style={{ margin: 0 }}>Automotive enquiries</h2>
        <p className="muted">Service requests sent to repair, wash, and tow workshops</p>
      </div>
      <div className="panel" style={{ overflowX: "auto" }}>
            ...
            {(list.data?.items ?? []).map((enquiry) => (
              <tr key={enquiry.id}>
                ...
                <td>
                  ...
                  <div className="muted">{enquiry.vehicleInfo || "—"}</div>
                </td>
                <td>
        ...
        {!list.isLoading && (list.data?.items.length ?? 0) === 0 ? (
          <EmptyList compact title="No automotive enquiries yet" />
        ) : null}
      </div>
    </div>
  );
