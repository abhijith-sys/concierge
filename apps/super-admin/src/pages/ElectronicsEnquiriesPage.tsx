...
export function ElectronicsEnquiriesPage() {
  const { user } = useAuth();
  const queryClient = useQueryClient();
  const params = new URLSearchParams({ pageSize: "50" });
  const list = useQuery({
    queryKey: ["admin", "electronics-enquiries", params.toString()],
    queryFn: () => api.electronicsEnquiries(params),
  ...
  const update = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["admin", "electronics-enquiries"] });
    },
  });
  ...
  return (
    <div className="stack">
      <div>
        <h2 style={{ margin: 0 }}>Electronics enquiries</h2>
        <p className="muted">Repair requests sent to device and IT shops</p>
      </div>
      <div className="panel" style={{ overflowX: "auto" }}>
            ...
            {(list.data?.items ?? []).map((enquiry) => (
              <tr key={enquiry.id}>
                ...
                <td>
                  ...
                  <div className="muted">{enquiry.deviceInfo || "—"}</div>
                </td>
                <td>
        ...
        {!list.isLoading && (list.data?.items.length ?? 0) === 0 ? (
          <EmptyList compact title="No electronics enquiries yet" />
        ) : null}
      </div>
    </div>
  );
