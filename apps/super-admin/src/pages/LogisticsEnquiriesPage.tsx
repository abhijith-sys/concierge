...
export function LogisticsEnquiriesPage() {
  const { user } = useAuth();
  const queryClient = useQueryClient();
  const params = new URLSearchParams({ pageSize: "50" });
  const list = useQuery({
    queryKey: ["admin", "logistics-enquiries", params.toString()],
    queryFn: () => api.logisticsEnquiries(params),
  ...
  return (
    <div className="stack">
      <div>
        <h2 style={{ margin: 0 }}>Logistics enquiries</h2>
        <p className="muted">Move and courier requests sent to operators and security crews</p>
      </div>
      <div className="panel" style={{ overflowX: "auto" }}>
            ...
            {(list.data?.items ?? []).map((enquiry) => (
              <tr key={enquiry.id}>
                ...
                <td>
                  ...
                  <div className="muted">{enquiry.packingRequired ? "Packing required" : "No packing"}</div>
                </td>
                <td>
        ...
        {!list.isLoading && (list.data?.items.length ?? 0) === 0 ? (
          <EmptyList compact title="No logistics enquiries yet" />
        ) : null}
      </div>
    </div>
  );
