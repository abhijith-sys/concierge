...
export function HomeTradeEnquiriesPage() {
  const { user } = useAuth();
  const queryClient = useQueryClient();
  const params = new URLSearchParams({ pageSize: "50" });
  const list = useQuery({
    queryKey: ["admin", "home-trade-enquiries", params.toString()],
    queryFn: () => api.homeTradeEnquiries(params),
  ...
  const update = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["admin", "home-trade-enquiries"] });
    },
  });
  ...
  return (
    <div className="stack">
      <div>
        <h2 style={{ margin: 0 }}>Home trade enquiries</h2>
        <p className="muted">Job requests sent to electricians, plumbers, and home trades</p>
      </div>
      <div className="panel" style={{ overflowX: "auto" }}>
            ...
            {(list.data?.items ?? []).map((enquiry) => (
              <tr key={enquiry.id}>
                ...
                <td>
                  ...
                  <div className="muted">{enquiry.jobLocation}</div>
                </td>
                <td>
                  {enquiry.serviceSelections.map((row) => `${row.name ?? "Package"} × ${row.quantity}`).join(", ")}
                </td>
                <td>
        ...
        {!list.isLoading && (list.data?.items.length ?? 0) === 0 ? (
          <EmptyList compact title="No home trade enquiries yet" />
        ) : null}
      </div>
    </div>
  );
