...
export function ProfessionalEnquiriesPage() {
  ...
  const list = useQuery({
    queryKey: ["admin", "professional-enquiries", params.toString()],
    queryFn: () => api.professionalEnquiries(params),
  ...
  const update = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["admin", "professional-enquiries"] });
    },
  });
  ...
  return (
    <div className="stack">
      <div>
        <h2 style={{ margin: 0 }}>Professional enquiries</h2>
        <p className="muted">Service requests sent to CAs, lawyers, and business consultants</p>
      </div>
      <div className="panel" style={{ overflowX: "auto" }}>
            ...
            {(list.data?.items ?? []).map((enquiry) => (
              <tr key={enquiry.id}>
                ...
                <td>
                  ...
                  <div className="muted">{enquiry.topic || "—"}</div>
                </td>
                <td>
                  {enquiry.serviceSelections.map((row) => `${row.name ?? "Service"} × ${row.quantity}`).join(", ")}
                </td>
                <td>
        ...
        {!list.isLoading && (list.data?.items.length ?? 0) === 0 ? (
          <EmptyList compact title="No professional enquiries yet" />
        ) : null}
      </div>
    </div>
  );
