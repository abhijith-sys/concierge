...
export function EventPackageModal({
  service,
  onClose,
  onEnquire,
}: {
  ...
  const packageType = displayValue(fieldByKey(service.fieldValues, "package_type")?.value);
  const guests = eventGuests(service);
  const duration = eventDuration(service);
  const includes = displayValue(fieldByKey(service.fieldValues, "package_includes")?.value);
      ...
      >
        ...
        <div className="grid gap-8 p-6 md:grid-cols-[1.25fr_0.75fr] md:p-8">
          <div>
            ...
            <h2 id="event-package-modal-title" className="mt-2 text-3xl font-extrabold tracking-tight text-ink">
              {service.name}
            </h2>
            {service.description ? (
            ...
            <dl className="mt-5 flex flex-wrap gap-x-8 gap-y-3 text-sm">
              {guests != null ? (
                <div>
                  <dt className="text-xs font-bold uppercase tracking-wide text-ink-soft/70">Guests</dt>
                  <dd className="mt-0.5 font-semibold text-ink">{guests}</dd>
                </div>
              ) : null}
              {duration != null ? (
                <div>
              ...
              {includes ? (
                <div>
                  <dt className="text-xs font-bold uppercase tracking-wide text-ink-soft/70">Includes</dt>
                  <dd className="mt-0.5 font-semibold text-ink">{includes}</dd>
                </div>
              ) : null}
            </dl>
          </div>
            ...
            <ul className="mt-4 grid gap-3 text-sm">
              {rates.hourly != null ? (
                <li className="flex justify-between gap-4">
                  <span>Hourly</span>
                  <strong>{formatEventMoney(rates.hourly, rates.currency)}</strong>
                </li>
              ) : null}
              {rates.day != null ? (
                <li className="flex justify-between gap-4">
                  <span>Day</span>
                  <strong>{formatEventMoney(rates.day, rates.currency)}</strong>
                </li>
              ) : null}
            </ul>
