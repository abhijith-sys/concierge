...
export function EventEnquiryForm({
  ...
  const create = useMutation({
    mutationFn: api.createEventEnquiry,
    onSuccess: () => {
      toast.success("Enquiry sent to the crew. They will contact you directly.");
  ...
  const selected = useMemo(() => packages.filter((item) => selectedIds.includes(item.id)), [packages, selectedIds]);
  ...
  function submit(event: FormEvent) {
    ...
    if (!form.eventDate || !form.venue.trim()) {
      toast.error("Add an event date and venue.");
      return;
    }
    create.mutate({
      businessId,
      ...
      packageSelections: selectedIds.map((serviceId) => ({ serviceId, quantity: quantities[serviceId] ?? 1 })),
    });
  }
  ...
  return (
    <form onSubmit={submit} className="mt-5 grid gap-4">
      <section>
        <p className="mb-2 text-[11px] font-extrabold uppercase tracking-[0.14em] text-white/50">Packages</p>
            ...
            return (
              <div key={item.id} className={`flex items-center gap-2.5 rounded-lg border px-3 py-2 text-sm ${checked ? "border-white/35 bg-white/10" : "border-white/10 bg-[#1c1c1c]"}`}>
                <label className="flex min-w-0 flex-1 cursor-pointer items-center gap-2.5">
                  <input type="checkbox" checked={checked} onChange={() => onTogglePackage(item.id)} className="size-4 accent-white" />
        ...
        <p className="mt-1.5 text-[11px] text-white/45">{selected.length ? `${selected.length} package${selected.length === 1 ? "" : "s"} selected` : "Tick a package."}</p>
      </section>
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <CompactField label="Event date">
          <Input type="date" required min={today} value={form.eventDate} onChange={(event) => setForm((current) => ({ ...current, eventDate: event.target.value }))} className={fieldClass} />
        </CompactField>
        <CompactField label="Start time">
          <Input type="time" value={form.eventTime} onChange={(event) => setForm((current) => ({ ...current, eventTime: event.target.value }))} className={fieldClass} />
        </CompactField>
        <CompactField label="Venue">
          <Input required value={form.venue} onChange={(event) => setForm((current) => ({ ...current, venue: event.target.value }))} placeholder="Hall, home, hotel…" className={`${fieldClass} placeholder:text-white/30`} />
        </CompactField>
        <CompactField label="Guests">
          <Input type="number" min={1} max={5000} required value={form.guests} onChange={(event) => setForm((current) => ({ ...current, guests: Number(event.target.value) || 1 }))} className={fieldClass} />
        </CompactField>
        ...
        <CompactField label="Notes (optional)">
          <Input value={form.notes} onChange={(event) => setForm((current) => ({ ...current, notes: event.target.value }))} placeholder="Theme, cuisine, timeline…" className={`${fieldClass} placeholder:text-white/30`} />
        </CompactField>
      </div>
