...
export function EventPackageCard({
  ...
  const highlights = eventCrewHighlights(listingFields);
  const rates = eventPackageRates(service);
  const badge = [packageType, categoryName].filter(Boolean).join(" · ") || "Event";
  const meta = [
    guests != null ? `${guests} guests` : null,
    duration != null ? `${duration} hrs` : null,
    includes,
  ]
    .filter(Boolean)
    .join(" | ");
        ...
        <div className="flex shrink-0 flex-col justify-between border-t border-line pt-3 sm:w-[200px] sm:border-l sm:border-t-0 sm:pl-5 sm:pt-0 lg:w-[220px]">
          {rates.day != null ? (
            <div className="rounded-md border border-gold/40 bg-cream px-2.5 py-2 text-[11px] leading-4">
              <p>
                <span className="font-extrabold text-navy">Day rate</span>
                <span className="text-ink-soft"> | {formatEventMoney(rates.day, rates.currency)}</span>
              </p>
            </div>
          ) : (
            <div />
          ...
          <div className="mt-4 text-right sm:mt-auto">
            <p className="text-2xl font-extrabold tracking-tight text-ink">
              {formatEventMoney(rates.from, rates.currency)}
            </p>
