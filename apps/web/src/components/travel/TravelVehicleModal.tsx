...
export function TravelVehicleModal({
  service,
  onClose,
  onEnquire,
}: {
  ...
  const vehicleType = displayValue(fieldByKey(service.fieldValues, "vehicle_type")?.value);
  const seats = travelSeats(service);
  const count = travelVehicleCount(service);
  const luggage = displayValue(fieldByKey(service.fieldValues, "luggage_capacity")?.value);
  const ac = fieldByKey(service.fieldValues, "ac")?.value === true;
  ...
  return (
    <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="travel-vehicle-modal-title"
      >
        ...
        <div className="grid gap-8 p-6 md:grid-cols-[1.25fr_0.75fr] md:p-8">
          <div>
            {vehicleType ? <p className="label-caps text-gold-dark">{vehicleType}</p> : null}
            <h2 id="travel-vehicle-modal-title" className="mt-2 text-3xl font-extrabold tracking-tight text-ink">
              {service.name}
            </h2>
            {service.description ? (
            ...
            <dl className="mt-5 flex flex-wrap gap-x-8 gap-y-3 text-sm">
              {seats != null ? (
                <div>
                  <dt className="text-xs font-bold uppercase tracking-wide text-ink-soft/70">Seats</dt>
                  <dd className="mt-0.5 font-semibold text-ink">{seats}</dd>
                </div>
              ) : null}
              <div>
                <dt className="text-xs font-bold uppercase tracking-wide text-ink-soft/70">AC</dt>
                <dd className="mt-0.5 font-semibold text-ink">{ac ? "Yes" : "No"}</dd>
              </div>
              {count != null ? (
                <div>
                  <dt className="text-xs font-bold uppercase tracking-wide text-ink-soft/70">In fleet</dt>
                  <dd className="mt-0.5 font-semibold text-ink">{count}</dd>
                </div>
              ) : null}
              {luggage ? (
                <div>
                  <dt className="text-xs font-bold uppercase tracking-wide text-ink-soft/70">Luggage</dt>
                  <dd className="mt-0.5 font-semibold text-ink">{luggage}</dd>
                </div>
              ) : null}
            </dl>
          </div>
            ...
            <ul className="mt-4 grid gap-3 text-sm">
              {rates.hourly != null ? (
                <li className="flex justify-between gap-4">
                  <span>Hourly</span>
                  <strong>{formatTravelMoney(rates.hourly, rates.currency)}</strong>
                </li>
              ) : null}
              {rates.airport != null ? (
                <li className="flex justify-between gap-4">
                  <span>Airport</span>
                  <strong>{formatTravelMoney(rates.airport, rates.currency)}</strong>
                </li>
              ) : null}
              {rates.outstation != null ? (
                <li className="flex justify-between gap-4">
                  <span>Outstation / day</span>
                  <strong>{formatTravelMoney(rates.outstation, rates.currency)}</strong>
                </li>
              ) : null}
              {rates.perKm != null ? (
                <li className="flex justify-between gap-4">
                  <span>Per km</span>
                  <strong>{formatTravelMoney(rates.perKm, rates.currency)}</strong>
                </li>
              ) : null}
              {rates.night != null ? (
                <li className="flex justify-between gap-4">
                  <span>Night charge</span>
                  <strong>{formatTravelMoney(rates.night, rates.currency)}</strong>
                </li>
              ) : null}
              {rates.waiting != null ? (
                <li className="flex justify-between gap-4">
                  <span>Waiting / hr</span>
                  <strong>{formatTravelMoney(rates.waiting, rates.currency)}</strong>
                </li>
              ) : null}
            </ul>
