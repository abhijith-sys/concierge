...
import { fieldNumber } from "../../lib/travel";
...
import { TravelEnquiryForm } from "./TravelEnquiryForm";
import { TravelVehicleCard } from "./TravelVehicleCard";
import { TravelVehicleModal } from "./TravelVehicleModal";
...
function ItemPagination({
  ...
  return (
    <nav className="mt-8 flex items-center justify-center gap-1.5" aria-label="Vehicle pages">
      <button
        type="button"
        disabled={page <= 1}
...
export function TravelOperatorView({
  profile,
  listing,
  vehicles,
  vehiclesLoading,
  ...
  const airports = displayValue(fieldByKey(operatorFields, "airports_served")?.value);
  const airportTransfer = fieldByKey(operatorFields, "airport_transfer")?.value === true;
  const outstation = fieldByKey(operatorFields, "outstation_available")?.value === true;
  ...
  const [openVehicle, setOpenVehicle] = useState<Service | null>(null);
  const [selectedIds, setSelectedIds] = useState<string[]>([]);
  const [itemPage, setItemPage] = useState(1);
  const itemPages = Math.max(1, Math.ceil(vehicles.length / ITEM_PAGE_SIZE));
  const pagedVehicles = vehicles.slice((itemPage - 1) * ITEM_PAGE_SIZE, itemPage * ITEM_PAGE_SIZE);
  ...
  function enquireVehicle(service: Service) {
  ...
  return (
    <>
      ...
      <section id="vehicles" className="scroll-mt-24 bg-surface-low">
        <div className="page-shell py-16 lg:py-24">
          <p className="label-caps text-ink-soft/70">{profile.name}</p>
          <h2 className="mt-3 text-3xl font-extrabold tracking-[-0.03em] text-ink md:text-4xl">Fleet & trips</h2>
          <p className="mt-3 max-w-2xl text-sm leading-7 text-ink-soft">
            Vehicles and packages with seats, luggage, and rates. Enquire to add one to your trip request.
          </p>
          {vehiclesLoading ? (
            ...
            <EmptyList
              ...
              description="This operator has not published a fleet yet."
            />
          )}
        </div>
      </section>
      ...
      <section className="page-shell py-16 lg:py-24">
        ...
        <dl className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="rounded-xl border border-line p-4">
            <dt className="label-caps text-ink-soft/70">Airport</dt>
            <dd className="mt-1 font-semibold">{airportTransfer ? airports || "Transfers available" : "Not offered"}</dd>
          </div>
          <div className="rounded-xl border border-line p-4">
            <dt className="label-caps text-ink-soft/70">Outstation</dt>
            <dd className="mt-1 font-semibold">{outstation ? "Available" : "Local only"}</dd>
          </div>
          {fleetSize != null ? (
            <div className="rounded-xl border border-line p-4">
              <dt className="label-caps text-ink-soft/70">Fleet</dt>
              <dd className="mt-1 font-semibold">{fleetSize} vehicles</dd>
            </div>
          ) : null}
          {languages ? (
            <div className="rounded-xl border border-line p-4">
      ...
      <section id="enquiry" className="scroll-mt-20 bg-[#111111] text-white">
        <div className="page-shell py-7 lg:py-9">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-xl font-extrabold tracking-[-0.03em] md:text-2xl">Send a trip request</h2>
      ...
      <section id="reviews" className="page-shell scroll-mt-20 py-12 lg:py-16">
        <p className="label-caps text-ink-soft/70">Passenger perspective</p>
        ...
        {reviewsLoading ? (
          ...
          <EmptyList compact className="mt-4" title="No reviews yet" description="Be the first to share a trip here." />
        )}
        <div className="mt-8">
          {canReview ? (
            <form onSubmit={onSubmitReview} className="rounded-xl bg-black p-6 text-white md:p-8">
              <h2 className="text-xl font-semibold">Share this trip</h2>
              <p className="mt-1 text-sm text-white/65">Your review helps others choose a ride confidently.</p>
              ...
              <Textarea
                ...
                rows={3}
                minLength={10}
                placeholder="Was pickup on time? Was the car as listed?"
            ...
            <div className="rounded-xl bg-surface-low p-6 md:flex md:items-center md:justify-between md:gap-6 md:p-7">
              <div>
                <h2 className="text-xl font-semibold">Rode with {profile.name}?</h2>
