...
export function ElectronicsEnquiryForm({
  businessId,
  packages,
  selectedIds,
  onTogglePackage,
  ...
  const create = useMutation({
    mutationFn: api.createElectronicsEnquiry,
    onSuccess: () => {
      toast.success("Enquiry sent to the repair shop. They will contact you directly.");
      setForm((current) => ({ ...current, notes: "", deviceInfo: "" }));
    },
    onError: (error) => {
  ...
  function submit(event: FormEvent) {
    ...
    if (!form.preferredDate) {
      toast.error("Add a preferred date.");
      return;
    }
    create.mutate({
      businessId,
      ...
      deviceInfo: form.deviceInfo.trim() || undefined,
  ...
  return (
    <form onSubmit={submit} className="mt-5 grid gap-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <CompactField label="Preferred date">
          <Input type="date" required min={today} value={form.preferredDate} onChange={(event) => setForm((current) => ({ ...current, preferredDate: event.target.value }))} className={fieldClass} />
        </CompactField>
        ...
        <CompactField label="Device info">
          <Input value={form.deviceInfo} onChange={(event) => setForm((current) => ({ ...current, deviceInfo: event.target.value }))} placeholder="Brand, model, issue…" className={`${fieldClass} placeholder:text-white/30`} />
        </CompactField>
        <CompactField label="Notes (optional)">
          <Input value={form.notes} onChange={(event) => setForm((current) => ({ ...current, notes: event.target.value }))} placeholder="Warranty, accessories…" className={`${fieldClass} placeholder:text-white/30`} />
        </CompactField>
