...
import {
  formatElectronicsMoney,
  electronicsDurationHours,
  electronicsPackageRates,
  electronicsRepairHighlights,
...
export function ElectronicsPackageCard({
  service,
  listingFields,
  categoryName,
  selected,
  ...
  const duration = electronicsDurationHours(service);
  const highlights = electronicsRepairHighlights(listingFields);
  const rates = electronicsPackageRates(service);
  const badge = [packageType, categoryName].filter(Boolean).join(" · ") || "Package";
        ...
        <div className="flex shrink-0 flex-col justify-between border-t border-line pt-3 sm:w-[200px] sm:border-l sm:border-t-0 sm:pl-5 sm:pt-0 lg:w-[220px]">
          {rates.job != null ? (
            <div className="rounded-md border border-gold/40 bg-cream px-2.5 py-2 text-[11px] leading-4">
              <p>
                <span className="font-extrabold text-navy">Job rate</span>
                <span className="text-ink-soft"> | {formatElectronicsMoney(rates.job, rates.currency)}</span>
              </p>
            </div>
          ) : (
            <div />
          ...
          <div className="mt-4 text-right sm:mt-auto">
            <p className="text-2xl font-extrabold tracking-tight text-ink">
              {formatElectronicsMoney(rates.from, rates.currency)}
            </p>
            <p className="mt-0.5 text-xs font-semibold text-ink-soft">
              {rates.hourly != null && rates.hourly === rates.from ? "per hour" : "per job"}
            </p>
            {selected ? (
