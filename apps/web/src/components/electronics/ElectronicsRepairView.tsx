...
import { ElectronicsEnquiryForm } from "./ElectronicsEnquiryForm";
import { ElectronicsPackageCard } from "./ElectronicsPackageCard";
import { ElectronicsPackageModal } from "./ElectronicsPackageModal";
...
export function ElectronicsRepairView({
  profile,
  listing,
  packages,
  packagesLoading,
  ...
  const repairFields = profile.fieldValues?.length ? profile.fieldValues : listing.fieldValues;
  const serviceHours = displayValue(fieldByKey(repairFields, "service_hours")?.value);
  const deviceTypes = displayValue(fieldByKey(repairFields, "device_types")?.value);
  const languages = displayValue(fieldByKey(repairFields, "languages")?.value);
  const cancellation = displayValue(fieldByKey(repairFields, "cancellation_policy")?.value);
  const whatsapp = displayValue(fieldByKey(repairFields, "whatsapp")?.value);
  ...
  return (
    <>
      ...
      <section id="packages" className="scroll-mt-24 bg-surface-low">
        <div className="page-shell py-16 lg:py-24">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-7 text-ink-soft">
            Device and IT repair packages with rates. Enquire to add one to your request.
          </p>
          {packagesLoading ? (
            ...
            <>
              <div className="mt-10 grid gap-4">
                {pagedItems.map((service) => (
                  <ElectronicsPackageCard
                    key={service.id}
                    service={service}
                    listingFields={repairFields}
            ...
            <EmptyList
              ...
              description="This shop has not published packages yet."
            />
          )}
        </div>
      </section>
      ...
      <section className="page-shell py-16 lg:py-24">
        <h2 className="text-3xl font-extrabold tracking-[-0.03em] text-ink">About this shop</h2>
        {copy.body || copy.lead ? (
        ...
        <dl className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          {deviceTypes ? (
            <div className="rounded-xl border border-line p-4">
              <dt className="label-caps text-ink-soft/70">Device types</dt>
              <dd className="mt-1 font-semibold">{deviceTypes}</dd>
            </div>
          ) : null}
          {languages ? (
            <div className="rounded-xl border border-line p-4">
      ...
      <section id="enquiry" className="scroll-mt-20 bg-[#111111] text-white">
        <div className="page-shell py-7 lg:py-9">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-xl font-extrabold tracking-[-0.03em] md:text-2xl">Send a repair request</h2>
      ...
      <section id="reviews" className="page-shell scroll-mt-20 py-12 lg:py-16">
        ...
        {reviewsLoading ? (
          ...
          <EmptyList compact className="mt-4" title="No reviews yet" description="Be the first to share a repair here." />
        )}
        <div className="mt-8">
          {canReview ? (
            <form onSubmit={onSubmitReview} className="rounded-xl bg-black p-6 text-white md:p-8">
              <h2 className="text-xl font-semibold">Share this shop</h2>
              <p className="mt-1 text-sm text-white/65">Your review helps others choose a repair shop confidently.</p>
            ...
            <div className="rounded-xl bg-surface-low p-6 md:flex md:items-center md:justify-between md:gap-6 md:p-7">
              <div>
                <h2 className="text-xl font-semibold">Repaired by {profile.name}?</h2>
