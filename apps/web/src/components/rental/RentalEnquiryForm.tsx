...
import { hireDaysBetween, rentalQuantity } from "../../lib/rentals";
import { Button, Input } from "../ui";

const MAX_ITEM_QTY = 100;
const fieldClass = "min-h-10 rounded-lg border-white/10 bg-[#1c1c1c] px-3 text-white";

function itemMax(service: Service) {
  const units = rentalQuantity(service);
  return Math.min(MAX_ITEM_QTY, Math.max(1, units ?? MAX_ITEM_QTY));
}
...
export function RentalEnquiryForm({
  ...
  const days = hireDaysBetween(form.hireFrom, form.hireTo);
  const create = useMutation({
    mutationFn: api.createRentalEnquiry,
    onSuccess: () => {
      toast.success("Enquiry sent to the shop. They will contact you directly.");
  ...
  const totalQty = selectedItems.reduce((sum, item) => sum + (quantities[item.id] ?? 1), 0);

  function setQuantity(id: string, value: number) {
    const item = items.find((row) => row.id === id);
    const max = item ? itemMax(item) : MAX_ITEM_QTY;
  ...
  function submit(event: FormEvent) {
    ...
    if (!selectedIds.length) {
      toast.error("Select at least one item to hire.");
      return;
    }
    if (days < 1) {
      toast.error("Choose a hire period.");
      return;
    }
    create.mutate({
      businessId,
      ...
      hireFrom: form.hireFrom,
      hireTo: form.hireTo,
      deliveryRequested: deliveryAvailable ? form.deliveryRequested : false,
      notes: form.notes.trim() || undefined,
      itemSelections: selectedIds.map((serviceId) => ({
        serviceId,
  ...
  return (
    <form onSubmit={submit} className="mt-5 grid gap-4">
      <section>
        <p className="mb-2 text-[11px] font-extrabold uppercase tracking-[0.14em] text-white/50">Items</p>
        ...
        <p className="mt-1.5 text-[11px] text-white/45">
          {selectedItems.length
            ? `${totalQty} item${totalQty === 1 ? "" : "s"} selected${days > 0 ? ` · ${days} day${days === 1 ? "" : "s"}` : ""}`
            : "Tick an item. Count starts at 1."}
        </p>
      </section>

      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <CompactField label="Hire from">
          <Input
            type="date"
            required
            min={today}
            value={form.hireFrom}
            onChange={(event) => setForm((current) => ({ ...current, hireFrom: event.target.value }))}
            className={fieldClass}
          />
        </CompactField>
        <CompactField label="Hire to">
          <Input
            type="date"
            required
            min={form.hireFrom || today}
            value={form.hireTo}
            onChange={(event) => setForm((current) => ({ ...current, hireTo: event.target.value }))}
            className={fieldClass}
          />
        </CompactField>
        ...
        {deliveryAvailable ? (
          <label className="flex items-end gap-2 pb-2 text-sm font-semibold text-white/80">
            <input
              ...
              onChange={(event) => setForm((current) => ({ ...current, deliveryRequested: event.target.checked }))}
              className="size-4 accent-white"
            />
            Request delivery
          </label>
        ...
        <CompactField label="Notes (optional)">
          <Input
            ...
            placeholder="Event date, delivery address…"
