...
import { AutomotiveEnquiryForm } from "./AutomotiveEnquiryForm";
import { AutomotivePackageCard } from "./AutomotivePackageCard";
import { AutomotivePackageModal } from "./AutomotivePackageModal";
...
export function AutomotiveWorkshopView({
  profile,
  listing,
  packages,
  packagesLoading,
  ...
  const workshopFields = profile.fieldValues?.length ? profile.fieldValues : listing.fieldValues;
  const serviceHours = displayValue(fieldByKey(workshopFields, "service_hours")?.value);
  const vehicleTypes = displayValue(fieldByKey(workshopFields, "vehicle_types")?.value);
  const languages = displayValue(fieldByKey(workshopFields, "languages")?.value);
  const cancellation = displayValue(fieldByKey(workshopFields, "cancellation_policy")?.value);
  const whatsapp = displayValue(fieldByKey(workshopFields, "whatsapp")?.value);
  ...
  const pagedItems = packages.slice((itemPage - 1) * ITEM_PAGE_SIZE, itemPage * ITEM_PAGE_SIZE);
  ...
  return (
    <>
      ...
      {canEdit && (profile.status === "pending" || profile.status === "rejected") ? (
        <div className="page-shell">
          <ApprovalBanner
            ...
            title={profile.status === "rejected" ? "This workshop needs changes" : "This workshop is awaiting review"}
          >
      ...
      <section id="packages" className="scroll-mt-24 bg-surface-low">
        <div className="page-shell py-16 lg:py-24">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-7 text-ink-soft">
            Repair, wash, and tow packages with rates. Enquire to add one to your request.
          </p>
          {packagesLoading ? (
            ...
            <>
              <div className="mt-10 grid gap-4">
                {pagedItems.map((service) => (
                  <AutomotivePackageCard
                    key={service.id}
                    service={service}
                    listingFields={workshopFields}
            ...
            <EmptyList
              ...
              description="This workshop has not published packages yet."
            />
          )}
        </div>
      </section>
      ...
      <section className="page-shell py-16 lg:py-24">
        <h2 className="text-3xl font-extrabold tracking-[-0.03em] text-ink">About this workshop</h2>
        {copy.body || copy.lead ? (
        ...
        <dl className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          {vehicleTypes ? (
            <div className="rounded-xl border border-line p-4">
              <dt className="label-caps text-ink-soft/70">Vehicle types</dt>
              <dd className="mt-1 font-semibold">{vehicleTypes}</dd>
            </div>
          ) : null}
          {languages ? (
            <div className="rounded-xl border border-line p-4">
      ...
      <section id="enquiry" className="scroll-mt-20 bg-[#111111] text-white">
        <div className="page-shell py-7 lg:py-9">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-xl font-extrabold tracking-[-0.03em] md:text-2xl">Send a workshop request</h2>
          ...
          <AutomotiveEnquiryForm
            businessId={profile.id}
            packages={packages}
            selectedIds={selectedIds}
            onTogglePackage={toggleItem}
          />
        </div>
      </section>
      ...
      <section id="reviews" className="page-shell scroll-mt-20 py-12 lg:py-16">
        ...
        {reviewsLoading ? (
          ...
          <EmptyList compact className="mt-4" title="No reviews yet" description="Be the first to share a service here." />
        )}
        <div className="mt-8">
          {canReview ? (
            <form onSubmit={onSubmitReview} className="rounded-xl bg-black p-6 text-white md:p-8">
              <h2 className="text-xl font-semibold">Share this workshop</h2>
              <p className="mt-1 text-sm text-white/65">Your review helps others choose a workshop confidently.</p>
              ...
              <Textarea
                ...
                rows={3}
                minLength={10}
                placeholder="Was the repair done well? Did the job match the listing?"
            ...
            <div className="rounded-xl bg-surface-low p-6 md:flex md:items-center md:justify-between md:gap-6 md:p-7">
              <div>
                <h2 className="text-xl font-semibold">Serviced by {profile.name}?</h2>
