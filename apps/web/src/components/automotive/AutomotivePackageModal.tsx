...
import { formatAutomotiveMoney, automotiveDurationHours, automotivePackageRates } from "../../lib/automotive";
import { Button } from "../ui";
import { SafeImage } from "../SafeImage";

export function AutomotivePackageModal({
  service,
  onClose,
  onEnquire,
}: {
  ...
  return (
    <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="automotive-package-modal-title"
      >
        ...
        <div className="grid gap-8 p-6 md:grid-cols-[1.25fr_0.75fr] md:p-8">
          <div>
            ...
            <h2 id="automotive-package-modal-title" className="mt-2 text-3xl font-extrabold tracking-tight text-ink">
              {service.name}
            </h2>
            {service.description ? (
            ...
            <ul className="mt-4 grid gap-3 text-sm">
              {rates.hourly != null ? (
                <li className="flex justify-between gap-4">
                  <span>Hourly</span>
                  <strong>{formatAutomotiveMoney(rates.hourly, rates.currency)}</strong>
                </li>
              ) : null}
              {rates.job != null ? (
                <li className="flex justify-between gap-4">
                  <span>Job</span>
                  <strong>{formatAutomotiveMoney(rates.job, rates.currency)}</strong>
                </li>
              ) : null}
            </ul>
