...
export function AutomotiveEnquiryForm({
  businessId,
  packages,
  selectedIds,
  onTogglePackage,
  ...
  const create = useMutation({
    mutationFn: api.createAutomotiveEnquiry,
    onSuccess: () => {
      toast.success("Enquiry sent to the workshop. They will contact you directly.");
      setForm((current) => ({ ...current, notes: "", vehicleInfo: "" }));
    },
    onError: (error) => {
  ...
  return (
    <form onSubmit={submit} className="mt-5 grid gap-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <CompactField label="Vehicle info">
          <Input value={form.vehicleInfo} onChange={(event) => setForm((current) => ({ ...current, vehicleInfo: event.target.value }))} placeholder="Make, model, year…" className={`${fieldClass} placeholder:text-white/30`} />
        </CompactField>
        <CompactField label="Notes (optional)">
          <Input value={form.notes} onChange={(event) => setForm((current) => ({ ...current, notes: event.target.value }))} placeholder="Issue, mileage…" className={`${fieldClass} placeholder:text-white/30`} />
        </CompactField>
