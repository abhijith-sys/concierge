...
import { fieldNumber } from "../../lib/health";
...
import { HealthEnquiryForm } from "./HealthEnquiryForm";
import { HealthServiceCard } from "./HealthServiceCard";
import { HealthServiceModal } from "./HealthServiceModal";
...
function ItemPagination({
  ...
  return (
    <nav className="mt-8 flex items-center justify-center gap-1.5" aria-label="Treatment pages">
      <button
        type="button"
        disabled={page <= 1}
...
export function HealthPracticeView({
  profile,
  listing,
  treatments,
  treatmentsLoading,
  ...
  const practiceFields = profile.fieldValues?.length ? profile.fieldValues : listing.fieldValues;
  const serviceHours = displayValue(fieldByKey(practiceFields, "service_hours")?.value);
  const specialties = displayValue(fieldByKey(practiceFields, "specialties")?.value);
  const homeVisit = fieldByKey(practiceFields, "home_visit")?.value === true;
  const languages = displayValue(fieldByKey(practiceFields, "languages")?.value);
  const cancellation = displayValue(fieldByKey(practiceFields, "cancellation_policy")?.value);
  const whatsapp = displayValue(fieldByKey(practiceFields, "whatsapp")?.value);
  const [openItem, setOpenItem] = useState<Service | null>(null);
  const [selectedIds, setSelectedIds] = useState<string[]>([]);
  const [itemPage, setItemPage] = useState(1);
  const itemPages = Math.max(1, Math.ceil(treatments.length / ITEM_PAGE_SIZE));
  const pagedItems = treatments.slice((itemPage - 1) * ITEM_PAGE_SIZE, itemPage * ITEM_PAGE_SIZE);
  ...
  return (
    <>
      ...
      {canEdit && (profile.status === "pending" || profile.status === "rejected") ? (
        <div className="page-shell">
          <ApprovalBanner
            ...
            title={profile.status === "rejected" ? "This practice needs changes" : "This practice is awaiting review"}
          >
      ...
      <section id="treatments" className="scroll-mt-24 bg-surface-low">
        <div className="page-shell py-16 lg:py-24">
          <p className="label-caps text-ink-soft/70">{profile.name}</p>
          <h2 className="mt-3 text-3xl font-extrabold tracking-[-0.03em] text-ink md:text-4xl">Treatments</h2>
          <p className="mt-3 max-w-2xl text-sm leading-7 text-ink-soft">
            Consultations, treatments, and wellness packages with rates. Enquire to add one to your request.
          </p>
          {treatmentsLoading ? (
            ...
            <EmptyList
              compact
              className="mt-10"
              title="No treatments listed yet"
              description="This practice has not published treatments yet."
            />
          )}
        </div>
      </section>
      ...
      <section className="page-shell py-16 lg:py-24">
        <h2 className="text-3xl font-extrabold tracking-[-0.03em] text-ink">About this practice</h2>
        {copy.body || copy.lead ? (
        ...
        <dl className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          {specialties ? (
            <div className="rounded-xl border border-line p-4">
              <dt className="label-caps text-ink-soft/70">Specialties</dt>
              <dd className="mt-1 font-semibold">{specialties}</dd>
            </div>
          ) : null}
          {homeVisit ? (
            <div className="rounded-xl border border-line p-4">
              <dt className="label-caps text-ink-soft/70">Home visit</dt>
      ...
      <section id="enquiry" className="scroll-mt-20 bg-[#111111] text-white">
        <div className="page-shell py-7 lg:py-9">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-xl font-extrabold tracking-[-0.03em] md:text-2xl">Send a health request</h2>
          ...
          <HealthEnquiryForm
            ...
            onToggleTreatment={toggleItem}
          />
        </div>
      </section>

      <section id="reviews" className="page-shell scroll-mt-20 py-12 lg:py-16">
        <p className="label-caps text-ink-soft/70">Patient perspective</p>
        ...
        {reviewsLoading ? (
          ...
          <EmptyList compact className="mt-4" title="No reviews yet" description="Be the first to share a visit here." />
        )}
        <div className="mt-8">
          {canReview ? (
            <form onSubmit={onSubmitReview} className="rounded-xl bg-black p-6 text-white md:p-8">
              <h2 className="text-xl font-semibold">Share this practice</h2>
              <p className="mt-1 text-sm text-white/65">Your review helps others choose care confidently.</p>
              ...
              <Textarea
                ...
                rows={3}
                minLength={10}
                placeholder="Was the practice on time? Did the treatment match the listing?"
            ...
            <div className="rounded-xl bg-surface-low p-6 md:flex md:items-center md:justify-between md:gap-6 md:p-7">
              <div>
                <h2 className="text-xl font-semibold">Visited {profile.name}?</h2>
