...
import { formatHealthMoney, healthDurationMinutes, healthServiceRates } from "../../lib/health";
import { Button } from "../ui";
import { SafeImage } from "../SafeImage";

export function HealthServiceModal({
  service,
  onClose,
  onEnquire,
}: {
    ...
    <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="grid gap-8 p-6 md:grid-cols-[1.25fr_0.75fr] md:p-8">
          <div>
            {treatmentType ? <p className="label-caps text-gold-dark">{treatmentType}</p> : null}
            <h2 id="health-service-modal-title" className="mt-2 text-3xl font-extrabold tracking-tight text-ink">
              {service.name}
            </h2>
            {service.description ? (
            ...
            <dl className="mt-5 flex flex-wrap gap-x-8 gap-y-3 text-sm">
              {duration != null ? (
                <div>
                  ...
                  <dd className="mt-0.5 font-semibold text-ink">{duration} min</dd>
                </div>
              ) : null}
            </dl>
          </div>
            ...
            <ul className="mt-4 grid gap-3 text-sm">
              {rates.session != null ? (
                <li className="flex justify-between gap-4">
                  <span>Session</span>
                  <strong>{formatHealthMoney(rates.session, rates.currency)}</strong>
                </li>
              ) : null}
              {rates.package != null ? (
                <li className="flex justify-between gap-4">
                  <span>Package</span>
                  <strong>{formatHealthMoney(rates.package, rates.currency)}</strong>
                </li>
              ) : null}
            </ul>
