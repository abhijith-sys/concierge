...
import { fieldNumber } from "../../lib/logistics";
...
import { LogisticsEnquiryForm } from "./LogisticsEnquiryForm";
import { LogisticsOfferingCard } from "./LogisticsOfferingCard";
import { LogisticsOfferingModal } from "./LogisticsOfferingModal";
...
function ItemPagination({
  ...
  return (
    <nav className="mt-8 flex items-center justify-center gap-1.5" aria-label="Service pages">
      <button
        type="button"
        disabled={page <= 1}
...
export function LogisticsOperatorView({
  profile,
  listing,
  offerings,
  offeringsLoading,
  ...
  const operatorFields = profile.fieldValues?.length ? profile.fieldValues : listing.fieldValues;
  const serviceHours = displayValue(fieldByKey(operatorFields, "service_hours")?.value);
  const coverage = displayValue(fieldByKey(operatorFields, "coverage_area")?.value);
  const packing = fieldByKey(operatorFields, "packing_available")?.value === true;
  const insurance = fieldByKey(operatorFields, "insurance_available")?.value === true;
  const fleetSize = fieldNumber(operatorFields, "fleet_size");
  const languages = displayValue(fieldByKey(operatorFields, "languages")?.value);
  const cancellation = displayValue(fieldByKey(operatorFields, "cancellation_policy")?.value);
  const whatsapp = displayValue(fieldByKey(operatorFields, "whatsapp")?.value);
  const [openOffering, setOpenOffering] = useState<Service | null>(null);
  const [selectedIds, setSelectedIds] = useState<string[]>([]);
  const [itemPage, setItemPage] = useState(1);
  const itemPages = Math.max(1, Math.ceil(offerings.length / ITEM_PAGE_SIZE));
  const pagedOfferings = offerings.slice((itemPage - 1) * ITEM_PAGE_SIZE, itemPage * ITEM_PAGE_SIZE);
  ...
  function enquireOffering(service: Service) {
  ...
  return (
    <>
      ...
      {canEdit && (profile.status === "pending" || profile.status === "rejected") ? (
        <div className="page-shell">
          <ApprovalBanner
            ...
            title={profile.status === "rejected" ? "This operator needs changes" : "This operator is awaiting review"}
          >
      ...
      <section id="offerings" className="scroll-mt-24 bg-surface-low">
        <div className="page-shell py-16 lg:py-24">
          <p className="label-caps text-ink-soft/70">{profile.name}</p>
          <h2 className="mt-3 text-3xl font-extrabold tracking-[-0.03em] text-ink md:text-4xl">Services</h2>
          <p className="mt-3 max-w-2xl text-sm leading-7 text-ink-soft">
            Courier runs, local moves, freight, and security shifts with rates. Enquire to add one to your request.
          </p>
          {offeringsLoading ? (
            ...
            <>
              <div className="mt-10 grid gap-4">
                {pagedOfferings.map((service) => (
                  <LogisticsOfferingCard
                    key={service.id}
                    service={service}
                    listingFields={operatorFields}
                    categoryName={listing.category?.name}
                    selected={selectedIds.includes(service.id)}
                    onOpen={setOpenOffering}
                    onEnquire={enquireOffering}
                  />
                ))}
              </div>
            ...
            <EmptyList
              ...
              description="This operator has not published offerings yet."
            />
          )}
        </div>
      </section>
      ...
      <section className="page-shell py-16 lg:py-24">
        <h2 className="text-3xl font-extrabold tracking-[-0.03em] text-ink">About this operator</h2>
        {copy.body || copy.lead ? (
        ...
        <dl className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          {coverage ? (
            <div className="rounded-xl border border-line p-4">
              <dt className="label-caps text-ink-soft/70">Coverage</dt>
              <dd className="mt-1 font-semibold">{coverage}</dd>
            </div>
          ) : null}
          <div className="rounded-xl border border-line p-4">
            <dt className="label-caps text-ink-soft/70">Packing</dt>
            <dd className="mt-1 font-semibold">{packing ? "Available" : "Not offered"}</dd>
          </div>
          <div className="rounded-xl border border-line p-4">
            <dt className="label-caps text-ink-soft/70">Insurance</dt>
            <dd className="mt-1 font-semibold">{insurance ? "Available" : "Not listed"}</dd>
          </div>
          {fleetSize != null ? (
            <div className="rounded-xl border border-line p-4">
              <dt className="label-caps text-ink-soft/70">Fleet</dt>
              <dd className="mt-1 font-semibold">{fleetSize} units</dd>
            </div>
          ) : null}
          {languages ? (
            <div className="rounded-xl border border-line p-4">
      ...
      <section id="enquiry" className="scroll-mt-20 bg-[#111111] text-white">
        <div className="page-shell py-7 lg:py-9">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-xl font-extrabold tracking-[-0.03em] md:text-2xl">Send a move request</h2>
      ...
      <section id="reviews" className="page-shell scroll-mt-20 py-12 lg:py-16">
        ...
        {reviewsLoading ? (
          ...
          <EmptyList compact className="mt-4" title="No reviews yet" description="Be the first to share a move here." />
        )}
        <div className="mt-8">
          {canReview ? (
            <form onSubmit={onSubmitReview} className="rounded-xl bg-black p-6 text-white md:p-8">
              ...
              <p className="mt-1 text-sm text-white/65">Your review helps others choose a mover confidently.</p>
              ...
              <Textarea
                ...
                rows={3}
                minLength={10}
                placeholder="Was pickup on time? Were goods handled carefully?"
      ...
      {openOffering ? (
        <LogisticsOfferingModal
          key={openOffering.id}
          service={openOffering}
