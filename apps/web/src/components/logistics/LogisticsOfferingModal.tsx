...
import {
  formatLogisticsMoney,
  logisticsCapacity,
  logisticsCrew,
  logisticsOfferingRates,
} from "../../lib/logistics";
import { Button } from "../ui";
import { SafeImage } from "../SafeImage";

export function LogisticsOfferingModal({
  service,
  onClose,
  onEnquire,
}: {
  ...
  const offeringType = displayValue(fieldByKey(service.fieldValues, "offering_type")?.value);
  const vehicleType = displayValue(fieldByKey(service.fieldValues, "vehicle_type")?.value);
  const capacity = logisticsCapacity(service);
  const crew = logisticsCrew(service);
    ...
    <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="grid gap-8 p-6 md:grid-cols-[1.25fr_0.75fr] md:p-8">
          <div>
            {offeringType ? <p className="label-caps text-gold-dark">{offeringType}</p> : null}
            <h2 id="logistics-offering-modal-title" className="mt-2 text-3xl font-extrabold tracking-tight text-ink">
              {service.name}
            </h2>
            {service.description ? (
            ...
            <dl className="mt-5 flex flex-wrap gap-x-8 gap-y-3 text-sm">
              {vehicleType ? (
                <div>
                  <dt className="text-xs font-bold uppercase tracking-wide text-ink-soft/70">Vehicle</dt>
                  <dd className="mt-0.5 font-semibold text-ink">{vehicleType}</dd>
                </div>
              ) : null}
              {capacity != null ? (
                <div>
                  <dt className="text-xs font-bold uppercase tracking-wide text-ink-soft/70">Capacity</dt>
                  <dd className="mt-0.5 font-semibold text-ink">{capacity} kg</dd>
                </div>
              ) : null}
              {crew != null ? (
                <div>
                  <dt className="text-xs font-bold uppercase tracking-wide text-ink-soft/70">Crew</dt>
                  <dd className="mt-0.5 font-semibold text-ink">{crew}</dd>
                </div>
              ) : null}
            </dl>
          </div>
            ...
            <ul className="mt-4 grid gap-3 text-sm">
              {rates.hourly != null ? (
                <li className="flex justify-between gap-4">
                  <span>Hourly</span>
                  <strong>{formatLogisticsMoney(rates.hourly, rates.currency)}</strong>
                </li>
              ) : null}
              {rates.day != null ? (
                <li className="flex justify-between gap-4">
                  <span>Day</span>
                  <strong>{formatLogisticsMoney(rates.day, rates.currency)}</strong>
                </li>
              ) : null}
              {rates.perKm != null ? (
                <li className="flex justify-between gap-4">
                  <span>Per km</span>
                  <strong>{formatLogisticsMoney(rates.perKm, rates.currency)}</strong>
                </li>
              ) : null}
            </ul>
