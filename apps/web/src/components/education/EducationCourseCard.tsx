...
export function EducationCourseCard({
  ...
  const highlights = educationInstituteHighlights(listingFields);
  const rates = educationCourseRates(service);
  const badge = [courseType, categoryName].filter(Boolean).join(" · ") || "Course";
  const meta = [
    batch != null ? `${batch} learners` : null,
    weeks != null ? `${weeks} weeks` : null,
    sessionHours != null ? `${sessionHours} hrs/session` : null,
  ]
    .filter(Boolean)
    .join(" | ");
        ...
        <div className="flex shrink-0 flex-col justify-between border-t border-line pt-3 sm:w-[200px] sm:border-l sm:border-t-0 sm:pl-5 sm:pt-0 lg:w-[220px]">
          {rates.course != null ? (
            <div className="rounded-md border border-gold/40 bg-cream px-2.5 py-2 text-[11px] leading-4">
              <p>
                <span className="font-extrabold text-navy">Course rate</span>
                <span className="text-ink-soft"> | {formatEducationMoney(rates.course, rates.currency)}</span>
              </p>
            </div>
          ) : (
            <div />
          ...
          <div className="mt-4 text-right sm:mt-auto">
            <p className="text-2xl font-extrabold tracking-tight text-ink">
              {formatEducationMoney(rates.from, rates.currency)}
            </p>
