...
export function HomeEnquiryForm({
  ...
  const create = useMutation({
    mutationFn: api.createHomeTradeEnquiry,
    onSuccess: () => {
      toast.success("Enquiry sent to the trade. They will contact you directly.");
  ...
  function submit(event: FormEvent) {
    ...
    if (!form.preferredDate || !form.jobLocation.trim()) {
      toast.error("Add a preferred date and job location.");
      return;
    }
    create.mutate({
      businessId,
  ...
  return (
    <form onSubmit={submit} className="mt-5 grid gap-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <CompactField label="Job location">
          <Input required value={form.jobLocation} onChange={(event) => setForm((current) => ({ ...current, jobLocation: event.target.value }))} placeholder="Site address or area…" className={`${fieldClass} placeholder:text-white/30`} />
        </CompactField>
        <CompactField label="Notes (optional)">
          <Input value={form.notes} onChange={(event) => setForm((current) => ({ ...current, notes: event.target.value }))} placeholder="Job details, access…" className={`${fieldClass} placeholder:text-white/30`} />
        </CompactField>
