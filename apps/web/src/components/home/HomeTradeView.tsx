...
import { fieldNumber } from "../../lib/home";
...
import { HomePackageModal } from "./HomePackageModal";
...
export function HomeTradeView({
  ...
  const tradeFields = profile.fieldValues?.length ? profile.fieldValues : listing.fieldValues;
  const serviceHours = displayValue(fieldByKey(tradeFields, "service_hours")?.value);
  const jobTypes = displayValue(fieldByKey(tradeFields, "job_types")?.value);
  const serviceRadius = fieldNumber(tradeFields, "service_radius_km");
  const languages = displayValue(fieldByKey(tradeFields, "languages")?.value);
  const cancellation = displayValue(fieldByKey(tradeFields, "cancellation_policy")?.value);
  const whatsapp = displayValue(fieldByKey(tradeFields, "whatsapp")?.value);
  ...
  return (
    <>
      ...
      {canEdit && (profile.status === "pending" || profile.status === "rejected") ? (
        <div className="page-shell">
          <ApprovalBanner
            ...
            title={profile.status === "rejected" ? "This trade needs changes" : "This trade is awaiting review"}
          >
      ...
      <section id="packages" className="scroll-mt-24 bg-surface-low">
        <div className="page-shell py-16 lg:py-24">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-7 text-ink-soft">
            Electrical, plumbing, and home job packages with rates. Enquire to add one to your request.
          </p>
          {packagesLoading ? (
            ...
            <EmptyList
              ...
              description="This trade has not published packages yet."
            />
          )}
        </div>
      </section>
      ...
      <section className="page-shell py-16 lg:py-24">
        <h2 className="text-3xl font-extrabold tracking-[-0.03em] text-ink">About this trade</h2>
        {copy.body || copy.lead ? (
        ...
        <dl className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          {jobTypes ? (
            <div className="rounded-xl border border-line p-4">
              <dt className="label-caps text-ink-soft/70">Job types</dt>
              <dd className="mt-1 font-semibold">{jobTypes}</dd>
            </div>
          ) : null}
          {serviceRadius != null ? (
            <div className="rounded-xl border border-line p-4">
              <dt className="label-caps text-ink-soft/70">Radius</dt>
              <dd className="mt-1 font-semibold">{serviceRadius} km</dd>
            </div>
          ) : null}
          {languages ? (
            <div className="rounded-xl border border-line p-4">
      ...
      <section id="enquiry" className="scroll-mt-20 bg-[#111111] text-white">
        <div className="page-shell py-7 lg:py-9">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-xl font-extrabold tracking-[-0.03em] md:text-2xl">Send a job request</h2>
      ...
      <section id="reviews" className="page-shell scroll-mt-20 py-12 lg:py-16">
        ...
        {reviewsLoading ? (
          ...
          <EmptyList compact className="mt-4" title="No reviews yet" description="Be the first to share a job here." />
        )}
        <div className="mt-8">
          {canReview ? (
            <form onSubmit={onSubmitReview} className="rounded-xl bg-black p-6 text-white md:p-8">
              <h2 className="text-xl font-semibold">Share this trade</h2>
              <p className="mt-1 text-sm text-white/65">Your review helps others choose a trade confidently.</p>
              ...
              <Textarea
                ...
                rows={3}
                minLength={10}
                placeholder="Was the work on time? Did the job match the listing?"
            ...
            <div className="rounded-xl bg-surface-low p-6 md:flex md:items-center md:justify-between md:gap-6 md:p-7">
              <div>
                <h2 className="text-xl font-semibold">Hired {profile.name}?</h2>
