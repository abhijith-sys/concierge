...
export function ProfessionalEnquiryForm({
  businessId,
  services,
  selectedIds,
  onToggleService,
  ...
  const create = useMutation({
    mutationFn: api.createProfessionalEnquiry,
    onSuccess: () => {
      toast.success("Enquiry sent to the firm. They will contact you directly.");
      setForm((current) => ({ ...current, notes: "", topic: "" }));
    },
    onError: (error) => {
  ...
  const selected = useMemo(() => services.filter((item) => selectedIds.includes(item.id)), [services, selectedIds]);
  ...
  return (
    <form onSubmit={submit} className="mt-5 grid gap-4">
            ...
            return (
              <div key={item.id} className={`flex items-center gap-2.5 rounded-lg border px-3 py-2 text-sm ${checked ? "border-white/35 bg-white/10" : "border-white/10 bg-[#1c1c1c]"}`}>
                <label className="flex min-w-0 flex-1 cursor-pointer items-center gap-2.5">
                  <input type="checkbox" checked={checked} onChange={() => onToggleService(item.id)} className="size-4 accent-white" />
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <CompactField label="Topic">
          <Input value={form.topic} onChange={(event) => setForm((current) => ({ ...current, topic: event.target.value }))} placeholder="Audit, filing, campaign…" className={`${fieldClass} placeholder:text-white/30`} />
        </CompactField>
        <CompactField label="Notes (optional)">
          <Input value={form.notes} onChange={(event) => setForm((current) => ({ ...current, notes: event.target.value }))} placeholder="Deadline, company size…" className={`${fieldClass} placeholder:text-white/30`} />
        </CompactField>
