...
import {
  formatProfessionalMoney,
  professionalDurationHours,
  professionalServiceRates,
} from "../../lib/professional";
import { Button } from "../ui";
import { SafeImage } from "../SafeImage";

export function ProfessionalServiceModal({
  service,
  onClose,
  onEnquire,
}: {
  ...
  const engagementType = displayValue(fieldByKey(service.fieldValues, "engagement_type")?.value);
  const duration = professionalDurationHours(service);
  ...
  return (
    <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="professional-service-modal-title"
      >
        ...
        <div className="grid gap-8 p-6 md:grid-cols-[1.25fr_0.75fr] md:p-8">
          <div>
            {engagementType ? <p className="label-caps text-gold-dark">{engagementType}</p> : null}
            <h2 id="professional-service-modal-title" className="mt-2 text-3xl font-extrabold tracking-tight text-ink">
              {service.name}
            </h2>
            {service.description ? (
            ...
            <ul className="mt-4 grid gap-3 text-sm">
              {rates.hourly != null ? (
                <li className="flex justify-between gap-4">
                  <span>Hourly</span>
                  <strong>{formatProfessionalMoney(rates.hourly, rates.currency)}</strong>
                </li>
              ) : null}
              {rates.retainer != null ? (
                <li className="flex justify-between gap-4">
                  <span>Retainer</span>
                  <strong>{formatProfessionalMoney(rates.retainer, rates.currency)}</strong>
                </li>
              ) : null}
              {rates.project != null ? (
                <li className="flex justify-between gap-4">
                  <span>Project</span>
                  <strong>{formatProfessionalMoney(rates.project, rates.currency)}</strong>
                </li>
              ) : null}
            </ul>
