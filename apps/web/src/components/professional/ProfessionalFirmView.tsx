...
import { ProfessionalEnquiryForm } from "./ProfessionalEnquiryForm";
import { ProfessionalServiceCard } from "./ProfessionalServiceCard";
import { ProfessionalServiceModal } from "./ProfessionalServiceModal";
...
export function ProfessionalFirmView({
  profile,
  listing,
  services,
  servicesLoading,
  ...
  const firmFields = profile.fieldValues?.length ? profile.fieldValues : listing.fieldValues;
  const serviceHours = displayValue(fieldByKey(firmFields, "service_hours")?.value);
  const practiceAreas = displayValue(fieldByKey(firmFields, "practice_areas")?.value);
  const remoteAvailable = fieldByKey(firmFields, "remote_available")?.value === true;
  const languages = displayValue(fieldByKey(firmFields, "languages")?.value);
  const cancellation = displayValue(fieldByKey(firmFields, "cancellation_policy")?.value);
  const whatsapp = displayValue(fieldByKey(firmFields, "whatsapp")?.value);
  const [openItem, setOpenItem] = useState<Service | null>(null);
  const [selectedIds, setSelectedIds] = useState<string[]>([]);
  const [itemPage, setItemPage] = useState(1);
  const itemPages = Math.max(1, Math.ceil(services.length / ITEM_PAGE_SIZE));
  const pagedItems = services.slice((itemPage - 1) * ITEM_PAGE_SIZE, itemPage * ITEM_PAGE_SIZE);
  ...
  return (
    <>
      ...
      {canEdit && (profile.status === "pending" || profile.status === "rejected") ? (
        <div className="page-shell">
          <ApprovalBanner
            ...
            title={profile.status === "rejected" ? "This firm needs changes" : "This firm is awaiting review"}
          >
      ...
      <section id="services" className="scroll-mt-24 bg-surface-low">
        <div className="page-shell py-16 lg:py-24">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-7 text-ink-soft">
            Advisory, legal, and consulting engagements with rates. Enquire to add one to your request.
          </p>
          {servicesLoading ? (
            ...
            <EmptyList
              ...
              description="This firm has not published services yet."
            />
          )}
        </div>
      </section>
      ...
      <section className="page-shell py-16 lg:py-24">
        <h2 className="text-3xl font-extrabold tracking-[-0.03em] text-ink">About this firm</h2>
        {copy.body || copy.lead ? (
        ...
        <dl className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          {practiceAreas ? (
            <div className="rounded-xl border border-line p-4">
              <dt className="label-caps text-ink-soft/70">Practice areas</dt>
              <dd className="mt-1 font-semibold">{practiceAreas}</dd>
            </div>
          ) : null}
          {remoteAvailable ? (
            <div className="rounded-xl border border-line p-4">
              <dt className="label-caps text-ink-soft/70">Remote</dt>
      ...
      <section id="enquiry" className="scroll-mt-20 bg-[#111111] text-white">
        <div className="page-shell py-7 lg:py-9">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-xl font-extrabold tracking-[-0.03em] md:text-2xl">Send a professional request</h2>
          ...
          <ProfessionalEnquiryForm
            businessId={profile.id}
            services={services}
            selectedIds={selectedIds}
            onToggleService={toggleItem}
          />
        </div>
      </section>
      ...
      <section id="reviews" className="page-shell scroll-mt-20 py-12 lg:py-16">
        ...
        {reviewsLoading ? (
          ...
          <EmptyList compact className="mt-4" title="No reviews yet" description="Be the first to share an engagement here." />
        )}
        <div className="mt-8">
          {canReview ? (
            <form onSubmit={onSubmitReview} className="rounded-xl bg-black p-6 text-white md:p-8">
              <h2 className="text-xl font-semibold">Share this firm</h2>
              <p className="mt-1 text-sm text-white/65">Your review helps others choose a firm confidently.</p>
              ...
              <Textarea
                ...
                rows={3}
                minLength={10}
                placeholder="Was the firm responsive? Did the service match the listing?"
            ...
            <div className="rounded-xl bg-surface-low p-6 md:flex md:items-center md:justify-between md:gap-6 md:p-7">
              <div>
                <h2 className="text-xl font-semibold">Worked with {profile.name}?</h2>
