...
export function ProfessionalServiceCard({
  ...
  const highlights = professionalFirmHighlights(listingFields);
  const rates = professionalServiceRates(service);
  const badge = [engagementType, categoryName].filter(Boolean).join(" · ") || "Service";
  const meta = [duration != null ? `${duration} hrs` : null].filter(Boolean).join(" | ");
        ...
        <div className="flex shrink-0 flex-col justify-between border-t border-line pt-3 sm:w-[200px] sm:border-l sm:border-t-0 sm:pl-5 sm:pt-0 lg:w-[220px]">
          {rates.retainer != null ? (
            <div className="rounded-md border border-gold/40 bg-cream px-2.5 py-2 text-[11px] leading-4">
              <p>
                <span className="font-extrabold text-navy">Retainer</span>
                <span className="text-ink-soft"> | {formatProfessionalMoney(rates.retainer, rates.currency)}</span>
              </p>
            </div>
          ) : (
            <div />
          ...
          <div className="mt-4 text-right sm:mt-auto">
            <p className="text-2xl font-extrabold tracking-tight text-ink">
              {formatProfessionalMoney(rates.from, rates.currency)}
            </p>
            <p className="mt-0.5 text-xs font-semibold text-ink-soft">
              {rates.hourly != null && rates.hourly === rates.from
                ? "per hour"
                : rates.project != null && rates.project === rates.from
                  ? "per project"
                  : "starting rate"}
            </p>
            {selected ? (
