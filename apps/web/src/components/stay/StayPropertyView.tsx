...
import { fieldList } from "../../lib/stays";
...
import { StayEnquiryForm } from "./StayEnquiryForm";
...
function RoomPagination({
  ...
  return (
    <nav className="mt-8 flex items-center justify-center gap-1.5" aria-label="Room pages">
      <button
        type="button"
        disabled={page <= 1}
...
export function StayPropertyView({
  profile,
  listing,
  rooms,
  roomsLoading,
  ...
  const stayFields = profile.fieldValues?.length ? profile.fieldValues : listing.fieldValues;
  const meals = fieldList(stayFields, "meals");
  const checkIn = displayValue(fieldByKey(stayFields, "check_in_time")?.value);
  const checkOut = displayValue(fieldByKey(stayFields, "check_out_time")?.value);
  const landmark = displayValue(fieldByKey(stayFields, "nearest_landmark")?.value);
  const cancellation = displayValue(fieldByKey(stayFields, "cancellation_policy")?.value);
  const whatsapp = displayValue(fieldByKey(stayFields, "whatsapp")?.value);
  const [openRoom, setOpenRoom] = useState<Service | null>(null);
  const [selectedIds, setSelectedIds] = useState<string[]>([]);
  const [roomPage, setRoomPage] = useState(1);
  const roomPages = Math.max(1, Math.ceil(rooms.length / ROOM_PAGE_SIZE));
  const pagedRooms = rooms.slice((roomPage - 1) * ROOM_PAGE_SIZE, roomPage * ROOM_PAGE_SIZE);

  useEffect(() => {
    setRoomPage((current) => Math.min(current, roomPages));
  }, [roomPages]);
  ...
  return (
    <>
      ...
      {canEdit && (profile.status === "pending" || profile.status === "rejected") ? (
        <div className="page-shell">
          <ApprovalBanner
            ...
            title={profile.status === "rejected" ? "This property needs changes" : "This property is awaiting review"}
          >
      ...
      <section id="rooms" className="scroll-mt-24 bg-surface-low">
        <div className="page-shell py-16 lg:py-24">
          <p className="label-caps text-ink-soft/70">{profile.name}</p>
          <h2 className="mt-3 text-3xl font-extrabold tracking-[-0.03em] text-ink md:text-4xl">Rooms & cottages</h2>
          <p className="mt-3 max-w-2xl text-sm leading-7 text-ink-soft">
            Photos, amenities, and nightly rates for each room. Enquire to add one to your request.
          </p>
          {roomsLoading ? (
            ...
            <>
              <div className="mt-10 grid gap-4">
                {pagedRooms.map((service) => (
                  <RoomCard
                    key={service.id}
                    service={service}
                    listingFields={stayFields}
                    categoryName={listing.category?.name}
                    rating={listing.avgRating}
                    reviewCount={listing.reviewCount ?? reviews.length}
                    selected={selectedIds.includes(service.id)}
                    onOpen={setOpenRoom}
                    onEnquire={enquireRoom}
                  />
                ))}
              </div>
              <RoomPagination page={roomPage} pages={roomPages} onPage={goToRoomPage} />
            </>
          ) : (
            <EmptyList
              compact
              ...
              description="This property has not published room or cottage options."
            />
          )}
        </div>
      </section>
      ...
      <section className="page-shell py-16 lg:py-24">
        <h2 className="text-3xl font-extrabold tracking-[-0.03em] text-ink">About this stay</h2>
        {copy.body || copy.lead ? (
        ...
        <dl className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {checkIn ? (
            <div className="rounded-xl border border-line p-4">
              <dt className="label-caps text-ink-soft/70">Check-in</dt>
              <dd className="mt-1 font-semibold">{checkIn}</dd>
            </div>
          ) : null}
          {checkOut ? (
            <div className="rounded-xl border border-line p-4">
              <dt className="label-caps text-ink-soft/70">Check-out</dt>
              <dd className="mt-1 font-semibold">{checkOut}</dd>
            </div>
          ) : null}
          {meals.length ? (
            <div className="rounded-xl border border-line p-4">
              <dt className="label-caps text-ink-soft/70">Meals</dt>
              <dd className="mt-1 font-semibold">{meals.join(", ")}</dd>
            </div>
          ) : null}
          {cancellation ? (
            <div className="rounded-xl border border-line p-4">
          ...
          {landmark ? (
            <div className="rounded-xl border border-line p-4 sm:col-span-2">
              <dt className="label-caps text-ink-soft/70">Nearby</dt>
              <dd className="mt-1 font-semibold">{landmark}</dd>
            </div>
          ) : null}
        </dl>
      ...
      <section className="page-shell pb-16 lg:pb-24">
        <StayAmenities propertyName={profile.name} listingFields={stayFields} />
      </section>
      ...
      <section id="enquiry" className="scroll-mt-20 bg-[#111111] text-white">
        <div className="page-shell py-7 lg:py-9">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-xl font-extrabold tracking-[-0.03em] md:text-2xl">Send a stay request</h2>
      ...
      <section id="reviews" className="page-shell scroll-mt-20 py-12 lg:py-16">
        ...
        {reviewsLoading ? (
          ...
          <EmptyList compact className="mt-4" title="No reviews yet" description="Be the first to share a stay here." />
        )}
        <div className="mt-8">
          {canReview ? (
            <form onSubmit={onSubmitReview} className="rounded-xl bg-black p-6 text-white md:p-8">
              <h2 className="text-xl font-semibold">Share your stay</h2>
              <p className="mt-1 text-sm text-white/65">Your review helps other guests choose confidently.</p>
              ...
              <Textarea
                ...
                rows={3}
                minLength={10}
                placeholder="What made the stay stand out?"
            ...
            <div className="rounded-xl bg-surface-low p-6 md:flex md:items-center md:justify-between md:gap-6 md:p-7">
              <div>
                <h2 className="text-xl font-semibold">Stayed at {profile.name}?</h2>
