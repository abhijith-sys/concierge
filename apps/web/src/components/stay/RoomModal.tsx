...
import { fieldNumber, formatStayMoney, occupancyLabel, stayRoomRates } from "../../lib/stays";
    ...
    <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="grid gap-8 p-6 md:grid-cols-[1.25fr_0.75fr] md:p-8">
          <div>
            {roomType ? <p className="label-caps text-gold-dark">{roomType}</p> : null}
            <h2 id="room-modal-title" className="mt-2 text-3xl font-extrabold tracking-tight text-ink">
              {service.name}
            </h2>
            {occupancy ? <p className="mt-2 text-sm font-semibold text-ink-soft">{occupancy}</p> : null}
            {service.description ? (
            ...
            {bedType || size || units ? (
              <dl className="mt-5 flex flex-wrap gap-x-8 gap-y-3 text-sm">
                {bedType ? (
                  <div>
                    <dt className="text-xs font-bold uppercase tracking-wide text-ink-soft/70">Bed</dt>
                    <dd className="mt-0.5 font-semibold text-ink">{bedType}</dd>
                  </div>
                ) : null}
                {size ? (
                  <div>
                    <dt className="text-xs font-bold uppercase tracking-wide text-ink-soft/70">Size</dt>
                    <dd className="mt-0.5 font-semibold text-ink">{size} sq ft</dd>
                  </div>
                ) : null}
                {units ? (
                  <div>
                    <dt className="text-xs font-bold uppercase tracking-wide text-ink-soft/70">Units</dt>
                    <dd className="mt-0.5 font-semibold text-ink">{units}</dd>
                  </div>
                ) : null}
              </dl>
            ) : null}
          ...
          <aside className="rounded-2xl bg-surface-low p-5">
            ...
            <ul className="mt-4 grid gap-3 text-sm">
              <li className="flex justify-between gap-4">
                <span>Weekday</span>
                <strong>{formatStayMoney(rates.weekday, rates.currency)} / night</strong>
              </li>
              {rates.weekend != null ? (
                <li className="flex justify-between gap-4">
                  <span>Weekend</span>
                  <strong>{formatStayMoney(rates.weekend, rates.currency)} / night</strong>
                </li>
              ) : null}
              {rates.extraPerson != null ? (
                <li className="flex justify-between gap-4">
                  <span>Extra person</span>
                  <strong>{formatStayMoney(rates.extraPerson, rates.currency)}</strong>
                </li>
              ) : null}
              {rates.extraBed != null ? (
                <li className="flex justify-between gap-4">
                  <span>Extra bed</span>
                  <strong>{formatStayMoney(rates.extraBed, rates.currency)}</strong>
                </li>
              ) : null}
            </ul>
        ...
        <div className="px-6 pb-6 md:px-8 md:pb-8">
          <StayAmenities propertyName={propertyName} listingFields={listingFields} roomFields={service.fieldValues} />
        </div>
      </div>
    </div>
  );
